...
while ($counter < $users){
    ...
    system ("curl -e http://localhost:8080 $val -F:name=tester$counter -Fpwd=test -FpwdConfirm=test http://admin:admin\@localhost:8080/system/userManager/user.create.html 2> /dev/null  >/dev/null;");
}
